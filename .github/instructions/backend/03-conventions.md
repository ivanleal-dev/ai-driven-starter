...
```csharp
// ✅ Correto - Interfaces de Repositório (Domain/Interfaces/Repositories/)
public interface IUsuarioRepository : IRepository<Usuario> { }
public interface IPedidoRepository : IRepository<Pedido> { }

// ✅ Correto - Interfaces de Domain Service (Domain/Interfaces/Services/)
...
### Objetos Comuns de Response (Application/Common/Results)

Estruturas reutilizáveis devem ser centralizadas em `Common/Results/`:

```csharp
// ✅ Correto - Usar o ErroValidacao comum
using TodoApp.Application.Common.Results;

public sealed record CriarClienteResponse
{
    public required bool Success { get; init; }
    public required string Message { get; init; }
    public ClienteCriadoData? Data { get; init; }
    public IReadOnlyList<ErroValidacao>? Errors { get; init; }  // ← Reutilizado
}

// ❌ Errado - Duplicar ErroValidacao em cada handler
public sealed record ErroValidacao { ... }  // Não duplicar!
```

**Objetos disponíveis em `Common/Results/`:**
- `Result<T>` / `Result` → Padrão de retorno de handlers
- `Error` → Erros de operação com código e mensagem
- `ErroValidacao` → Erro de validação padronizado para responses (Campo + Mensagem)
...
[Route("api/[controller]")]
public sealed class PedidosController : BaseController { }
