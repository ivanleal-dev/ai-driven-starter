    ...
    IValidator<CriarUsuarioRequest> validator)
{
        ...
        CriarUsuarioRequest request, CancellationToken ct)
    {
        ...
        if (!validation.IsValid)
            return Result<UsuarioResponse>.Failure(Error.Validation(validation));
...
// NEW: Controllers/v1/UsuariosController.cs
[ApiController]
    ...
    CriarUsuarioRequest request, CancellationToken ct)
{
    ...
    if (!validation.IsValid)
        return Result<UsuarioResponse>.Failure(
            Error.Validation(ExtractValidationErrors(validation)));
...
## 5️⃣ Repository.AddAsync/SaveAsync → UnitOfWork Pattern
...
### ✅ Padrão Novo: UnitOfWork

```csharp
// NEW: UnitOfWork centralizado
var usuario = new Usuario { Email = "user@example.com" };
...
- [ ] ❌ SaveAsync por Repositório → ✅ UnitOfWork Centralizado
